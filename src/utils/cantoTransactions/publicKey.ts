...
import { ethToCanto, getSenderObj, signAndBroadcastTxMsg } from './helpers';
...
export async function generatePubKey(hexAddress: string | undefined, setIsSuccess : (s :string) => void) {
    ...
    const bech32Address = await ethToCanto(hexAddress, CantoMainnet.cosmosAPIEndpoint);
    if (!bech32Address) {
        setIsSuccess("could not find user")
        return;
    }
